function [imq_dic,imq_cell,imq_struct]= dailyQAAnalyzerLaser4(image_dir )
%{
...
for k=1:length(dates)
    ...
    if ~any(isnan(im_quality)) % only cell structure was added laser position.
       ...
       image_quality_cell{k,9}=laser_x; % added three laser position.
       
       image_quality_cell{k,10}=laser_y;
       
       image_quality_cell{k,11}=laser_z;
   ...
   image_quality_cell=reshape(image_quality_cell,[],11); % reshape it to have 8 coloumn.
   ...
   % close all figures after analysis done
