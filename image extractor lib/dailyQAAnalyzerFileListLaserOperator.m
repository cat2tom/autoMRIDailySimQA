function [imq_dic,imq_cell,op_cell,imq_struct]= dailyQAAnalyzerFileListLaserOperator(image_file_list)
%{
...
Input: image_file_list-a list of QA files just added into the directory.
...
for k=1:length(dates)
    ...
    [laser_x,laser_y,laser_z]=getLaserCoordinates(center);% use the center slice to get laser position.
    ...
    im_quality=[SNR  uniformity  contrast ghosting   distance_mm_45  distance_mm_135  output2,laser_x,laser_y,laser_z] % added laser support.
    ...
    if ~any(isnan(im_quality))
       ...
       image_quality_cell{k,4}=round(contrast,3);
       
       image_quality_cell{k,5}=round(ghosting,4);
       
       image_quality_cell{k,6}=round(distance_mm_45,3);
       
       image_quality_cell{k,7}=round(distance_mm_135,3);
        
       image_quality_cell{k,8}=round(output2,3);
       
       image_quality_cell{k,9}=round(laser_x,3); % added three laser positions.
       
       image_quality_cell{k,10}=round(laser_y,3);
       
       image_quality_cell{k,11}=round(laser_z,3);
   ...
   image_quality_cell=reshape(image_quality_cell,[],11); % reshape it to have 11 coloumn.
