function excel_file_name = writeDailyQAToExcelFileLaser(excel_file_name,daily_QA_results )
%{
...
Input: 
       ...
       written.
       daily_QA_resutls-cell arrray containig QA resutls. 
...
if exist(excel_file_name,'file')
    ...
    if ~strcmp(old_results(1,1),'Date')
        ...
        'Diagnal Distance(45 degree}','Diagnal Distance (145)','Output','Laser X', 'Laser Y','Laser Z'}; % added laser coordinates.
...
%     added_range=strcat('A',num2str(row+1),':','H',num2str(row1+row)); % define a new square area for new data.
    added_range=strcat('A1',':','K',num2str(row1+row));
    
    %Merge the new data into old one.
